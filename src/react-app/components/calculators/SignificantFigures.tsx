...
export const SignificantFigures: React.FC = () => {
  ...
  const handleCalculate = () => {
    ...
    const roundedNumberString = num.toPrecision(sf);

    let coefficient: string;
    let exponent: string;

    if (roundedNumberString.includes('e')) {
        [coefficient, exponent] = roundedNumberString.split('e');
    } else {
        const roundedNum = parseFloat(roundedNumberString);
        const exponentialString = roundedNum.toExponential();
        [coefficient, exponent] = exponentialString.split('e');
    }
  ...
  return (
    <div className="p-6 space-y-6">
        <div className="space-y-4">
            ...
            <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Significant Figures</label>
                <select
                    value={sf}
